...
export class InstanceTypeInstancesComponent implements OnInit, OnDestroy {
  ...
  constructor(private instanceTypeService: InstanceTypeService, private route: ActivatedRoute, private sortPipe: SortPipe, private instanceService: InstanceService) {
  }
  ...
  private subscribeToInstanceHealth() {
    ...
    this.subscription.connect({}, function() {
      that.subscription.subscribe("/instancestatus", (data) => that.updateInstanceStatus(JSON.parse(data.body)));
    });
  }
